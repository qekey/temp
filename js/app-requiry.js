...
var app = angular.module("inquiry-app", ["ngSanitize","ngTouch", "angucomplete-alt"]);
    ...
    app.controller('InquiryListController', function ($scope, $http) {
      ...
      $http({   url:'http://192.168.1.103:8080/demo-service/api/requiry_list/requiry_id',
                method:'GET'
  ...
  app.directive('mfsInput',function($http){
      ...
      return {
              ...
              link:function(scope, ele, attrs){
                ...
                $http({
                url:'http://192.168.1.103:8080/demo-service/api/mfrs/search?pn='+scope.model,
                method:'GET'
                }).success(function(data,header,config,status){
  ...
  app.directive("pnInput",function($http){
      return {
        ...
        link:function(scope, elements, attrs){
               ...
               $http({
                url:'http://192.168.1.103:8080/demo-service/api/inventory/find?pn='+this.value,
                method:'GET'
                }).success(function(data,header,config,status){
                    //响应成功    
